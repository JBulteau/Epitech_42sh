...
shell_t *shell)
{
	...
	comm = \
	set_separator(comm, node, (int[2]){comm_index[0], new_index}, shell);
	return (comm);
}
...
shell_t *shell)
{
	...
	comm = apply_separator(comm, (node_t*[2]){node->next[idx[0]]->\
	next[idx[1]]->next[idx[2]], node}, (int[2]){sep, idx[3]}, shell);
...
shell_t *shell)
{
	...
	&& node->next[idx[0]]->next[idx[1]]->next[idx[2]]) {
		...
		next[idx[2] - 1]->separator >= S_AMPERSAND) {
			comm = handle_advanced_separators(comm, node, \
			(int[4]){idx[0], idx[1], idx[2], idx[3]}, shell);
		} else if (idx[1] > 0 && node->next[idx[0]]->\
		next[idx[1] - 1]->separator <= D_PIPE && node->\
		next[idx[0]]->next[idx[1] - 1]->separator >= SEMICOLON) {
			comm = handle_basic_separators(comm, node, \
			(int[4]){idx[0], idx[1], idx[2], idx[3]}, shell);
			break;
		} else {
			break;

...
int fill_result_correct(char **result, glob_t *pglob, int i)
{
	free(*result);
	*result = strdup(pglob->gl_pathv[i] + 2);
	if (!(*result))
		return (1);
	return (0);
}
...
int correct_long(char **result, glob_t *pglob)
{
	...
	for (int i = 0; pglob->gl_pathv[i] != NULL; i++) {
		if ((100 - ((int)strlen(pglob->gl_pathv[i] + 2) * 100) / (int)strlen(*result)) < 0 || \
100 - (strlen(pglob->gl_pathv[i] + 2) * 100) / strlen(*result) > 30)
			continue;
		both[1] = pglob->gl_pathv[i] + 2;
		count_correct = \
count_correct_letter(distance_allowed, both);
		if (((count_correct * 100) / strlen(*result)) > 70)
			return (fill_result_correct(result, pglob, i));
	}
	return (1);
}

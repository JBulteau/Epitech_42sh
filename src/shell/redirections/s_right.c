...
{
	comm->red[S_RIGHT]->fd[0] = open(comm->red[S_RIGHT]->target, O_RDWR | O_CREAT | O_TRUNC, 0600);
	comm->red[S_RIGHT]->fd[1] = dup(STDOUT_FILENO);

	if (comm->red[S_RIGHT]->fd[0] == -1 || comm->red[S_RIGHT]->fd[1] == -1 || dup2(comm->red[S_RIGHT]->fd[0], STDOUT_FILENO) == -1)
		return (-1);
	return (0);
}
...
int end_s_right(comm_t *comm)
{
	...
	dup2(comm->red[S_RIGHT]->fd[1], STDOUT_FILENO);
